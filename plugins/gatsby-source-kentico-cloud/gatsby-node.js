...
exports.sourceNodes = async ({ actions, createNodeId }, { kcProjectId, kcLanguageCodenames }) => {
  ...
  let nonDefaultLanguagePromises = kcLanguageCodenames
    .filter(languageCodename => languageCodename !== defaultLanguageCodename)
  ...
  languageResponses.forEach(languageResponse => {
    ...
    contentItemNodes.forEach(contentItemNode => {
      ...
      if (languageVariantItem !== undefined && languageVariantItem !== null) {
        ...
        const languageVariantNode = normalize.createContentItemNode(createNodeId, languageVariantItem, contentTypeNodes)
        normalize.decorateItemNodeWithLanguageVariantLink(languageVariantNode, contentItemNodes)
        allNodesOfCurrentLanguage.push(languageVariantNode)
      }
    })

    if (languageCodename !== null) {
      nonDefaultLanguageItemNodes.set(languageCodename, allNodesOfCurrentLanguage)
    }
  })

  for (let [languageCodename, currentLanguageNodes] of nonDefaultLanguageItemNodes) {
    contentItemNodes.forEach(contentItemNode => {
      normalize.decorateItemNodeWithLanguageVariantLink(contentItemNode, currentLanguageNodes)
    })

    for (let [otherLanguageCodename, otherLanguageNodes] of nonDefaultLanguageItemNodes) {
      if (otherLanguageCodename !== languageCodename) {
        currentLanguageNodes.forEach(contentItemNode => {
          normalize.decorateItemNodeWithLanguageVariantLink(contentItemNode, otherLanguageNodes)
          normalize.decorateItemNodeWithLanguageVariantLink(contentItemNode, contentItemNodes)
        })
      }
    }

    normalize.decorateTypeNodesWithItemLinks(currentLanguageNodes, contentTypeNodes)
  }
  ...
  nonDefaultLanguageItemNodes.forEach(languageNodes => {
    languageNodes.forEach(itemNode => {
      try {
        ...
        console.log(`Error when creating language variant nodes. Details: ${error}`)
      }
    })
  })
